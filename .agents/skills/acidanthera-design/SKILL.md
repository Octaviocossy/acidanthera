...
- Geist is for UI chrome. **Mono is for source, sans is for rendered prose**: JetBrains Mono carries the editor, code blocks, inline code, and all metadata, while the *read view* sets rendered prose in `--font-sans`. A code block uses the `--font-mono` **token**, never `settings.editorFont` — the read view is not the editor.
- The read view's *note header block* and body share one **~680px measure**, so the breadcrumb, the title, and the first paragraph start on a single left edge. Headings stay capped at weight 500 there as everywhere else.
...
The ember accent means **the AI acted here — or this navigates into the vault** (ADR 0105, widened by ADR 0126). It is permitted only for the primary nav's **Agent** `✦`, Send, the active model pill, a running tool chip, an agent-turn glyph, a dirty-note dot, and a `[[wikilink]]` whose target **resolves** — and, at disabled opacity, an AI action that is offered but not yet available. Do not use it for success, status, decoration, or large fills.

The wikilink entry carries its own boundary, and the boundary is what keeps the widening narrow: a link that is **missing or ambiguous** gets **no ember at all** — it renders muted and struck through, and is not clickable. The three states are identical in the read view and in the editor's decoration, because the same link changing color under a view toggle would be worse than either color alone. Resolution is a scan of the cached vault tree, never an index, and an ambiguous basename is *marked, never guessed*.
...
- There is **no titlebar and no status bar**. The 40px top band is the *chrome strip*, which each region renders for itself; it carries **no state**, and the only controls it may carry are those that act on *what the window is currently showing* (ADRs 0107, 0121, amended by 0037). Exactly two do so far: the **navigation-history** back/forward pair on the sidebar's strip, and the **view toggle** on the viewer's. Every *global* control lives in the sidebar — the primary nav, the brand row, the footer identity block, or, while collapsed, the rail. New state goes on the surface that owns it; a shared bar is not an option.
...
- A clipped surface carries a hover reveal, and inside the sidebar that reveal is the drawn `Tooltip` primitive, not a native `title` (ADR 0120).
...
Honor invariant 21: the AI accent marks AI agency **or a link into the vault** (ADR 0126) and nothing else, with the brand mark exempt as identity rather than signal (ADR 0122) and the dirty-note dot as the one permitted status indicator. Honor invariants 36-38: one markdown parser serves both views, the read view's only write is the task checkbox, and a wikilink has three states that mean the same thing in either view. Honor invariant 22: acidanthera names are the only token vocabulary; Factory names and aliases no longer exist. Honor invariant 23: there is no status bar and no titlebar. Honor invariant 27: `--danger` marks the destructive path only, which together with invariant 21 leaves the app exactly two colored fills.
