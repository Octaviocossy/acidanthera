...
The ember accent means **the AI acted here — or this navigates into the vault** (ADR 0007, widened by ADR 0040). It is permitted only for the primary nav's **Agent** `✦`, Send, the active model pill, a running tool chip, an agent-turn glyph, a dirty-note dot, and a `[[wikilink]]` whose target **resolves** — and, at disabled opacity, an AI action that is offered but not yet available. Do not use it for success, status, decoration, or large fills.
...
- There is **no titlebar and no status bar**. The 40px top band is the *chrome strip*, which each region renders for itself; it carries **no state**, and the only controls it may carry are those that act on *what the window is currently showing* (ADRs 0009, 0035, amended by 0037). Exactly two do so far: the **navigation-history** back/forward pair on the sidebar's strip, and the **view toggle** on the viewer's. Every *global* control lives in the sidebar — the primary nav, the brand row, the footer identity block, or, while collapsed, the rail. New state goes on the surface that owns it; a shared bar is not an option.
