...
|---|----------|--------|-----------|
| 1 | Menu rendering | App-drawn React overlay in `Layout`, at click coords, viewport-clamped | Native `Menu::popup` is untestable in jsdom, un-themeable, and needs a two-language round trip per action (ADR 0111). It must not mount in `Sidebar` — `overflow-y-auto` inside an `overflow-hidden` row would clip it |
| 2 | Where a deleted entry goes | OS trash via the `trash` crate; never `fs::remove_*` | Orbit's content is hand-written notes and there is no undo (ADR 0110) |
...
| 5 | Collapsed *sidebar rail* | No context menu — expanded sidebar only | 40px glyphs identified by hover tooltip alone; ADR 0109 frames the rail as a launcher, and it has no draft row to render a create into |
...
| 8 | The dead `modal` layer | Wake it, and add **swallow semantics**: `'modal'` first in `LAYER_PRECEDENCE`, absorbing every keydown while active | Fixes the leak properly instead of adding a fourth hardcoded dialog (ADR 0112) |
...
| 15 | Destructive affordance | **No color** — a hairline rule above Delete; the dialog is the affordance | ADR 0105 reserves the accent for AI agency, and `ToastHost` already conveys "error" without color. A `--danger` token adds a third color semantic for one already-gated item; the diff red means "removed line inside a diff" |
...
- **Retrofitting `CloseBufferDialog` / `SwitchVaultDialog` / `SettingsDialog` onto the `modal`
  ...
  Recorded in ADR 0112 as known and deferred.
- **A "don't ask again" preference.**
- **In-app undo or restore-from-trash.** Finder is the recovery path — that is the point of ADR 0110.
...
- `.agents/adr/0111-context-menu-is-app-drawn.md` — note this cuts the opposite way from ADR 0106,
