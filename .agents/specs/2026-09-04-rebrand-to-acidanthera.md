...
| 3 | The mark's ember ring vs. invariant 21 | Ember on the **app icon and favicon only**; every in-app rendering monochrome. Invariant 21 gains one clause; ADR 0105 stands | Amending ADR 0105 was the alternative and was rejected: the *chat toggle*'s ember only pays while it is the sole accent pixel in chrome, which was the whole argument for putting it there. See ADR 0117 |
| 4 | How the identity renders in-app | Hand-drawn SVG mark (`AcidantheraMarkGlyph`) + the word in **Geist**, no bundled brand typeface | No font dependency, no raster in chrome, and ADR 0115's single-hand-drawn-exception shape is preserved rather than widened |
...
| 11 | Favicon | New **SVG favicon from the mark, ember included**; `vite.svg` and `tauri.svg` deleted | `index.html:5` still ships Vite's template logo on a live surface. Ember-bearing is the consistent reading of ADR 0117 — the test is "never renders inside the window", which a favicon passes exactly as the app icon does. A favicon cannot inherit `currentColor`, so it needs fixed ink |
...
0106). The collision is adjacency, not confusion of function: a notes app against a bootloader
...
- ***AI accent*** — the app-icon/favicon exception added, with the "never renders inside the
  window" test stated as what draws the boundary. ADR 0109 decision 16's reference generalised
  from "the orbit mark" to "the brand mark".
- **Invariant 21** — same exception, citing ADR 0117; `AcidantheraMarkGlyph` named as
  monochrome.
...
- `.agents/adr/0117-app-icon-is-not-app-chrome.md` — the ember exception, with ADR 0105 left
  standing
- `.agents/adr/0118-rename-reaches-the-on-disk-format.md` — why a cosmetic rename changed a
