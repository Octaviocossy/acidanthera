...
> Amended: 2026-09-06 — decision 12 partitioned by ADR 0120 (concurrent sidebar-tooltip work)
...
| 2 | Not a fourth focus region | Rejected | Would mean rewriting `reachableRegions`, the `Ctrl-w h/l` cycle and invariants 1 and 24 for a table that is read and dismissed. ADR 0107 exists to interrupt exactly this reflex |
| 3 | Read-only, not a rebinding UI | Read-only, with one control that opens `keymaps.toml` as a config buffer | ADR 0101 leaves the file authoritative and ADR 0103's own consequence says the seeded, fully-commented catalog *is* the rebinding UX. A UI writer would be a second writer |
...
- **Rebinding from the UI** — chord capture, live conflict preview, a `toml_edit` write path. `keymaps.toml` stays the single writer (ADR 0101).
...
real candidate — *read-only rather than a rebinding UI* — **applies** ADR 0101 (the config file
is authoritative) rather than deciding something new, the same reasoning already recorded for the
accent-in-chrome rule under ADR 0105. *No chord is a string literal* is obviously right once
...
concurrent tooltip work (ADR 0120) takes the sidebar's six. Note `Viewer.test.tsx` asserts
