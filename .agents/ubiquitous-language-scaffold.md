...
| Scaffold sync | process | *install*, *copy*, *scaffold update* | What `/install-scaffold` does on a re-run: creates what the target lacks and updates the scaffold-owned paths still holding its last delivery. Project-owned paths are left alone; a scaffold-owned path that diverges is conflict-marked — in place for a regular file, beside it for a symlink or directory — and the run exits non-zero for `resolving-scaffold-sync` (ADR-0019). A path beneath an ancestor that is not a real directory is refused unread and reported as a conflict, unmarked. The sync never resolves a divergence itself. Not a git merge — the target need not be a repository. |
| Sync baseline | generated file | *lockfile*, *receipt*, *state file* | `<target>/.agents/scaffold.baseline` — what the two sides last agreed on, per path: the digest the scaffold delivered, the digest the destination had at that agreement, and whether markers are outstanding. It is the only thing that tells "the scaffold moved and the target did not" apart from "the target edited this", and a resolved conflict from a new edit — all the same observation without it. An entry whose two digests differ with no marker outstanding is an *agreed divergence*. Written by every sync, committed by the target, never hand-edited. |
...
| Agreed divergence | sync baseline state | *blessed divergence*, *merged resolution*, *kept resolution* | A scaffold-owned path whose baseline entry holds a destination digest different from the delivered one, with no marker outstanding: a resolution that kept project content, accepted as what the two sides agree on. Reported `= unchanged` while the scaffold stands still; when the scaffold moves it conflicts again and is never delivered over (ADR-0022). A resolution that took the scaffold's version verbatim is not one — it lands as identical. |
| Artifact | concept | *deliverable*, *document*, *output* | Anything an agent writes for a later reader: a committed file (spec, plan, ADR, rule, command, skill, glossary, code, comment, test), a commit message, a GitHub issue, PR or comment, a handoff summary, a review report. Not the conversation, which is held in the user's language. Not a generated file — the corpus pack, the glossary index, the sync baseline, a runner `.diff` — which carries whatever its sources carry and is never translated or rewritten. |
...
| Hard violation | finding severity | *error* | A breach of an invariant the repo committed to: `.agents/ubiquitous-language-invariants.md`, `.agents/ubiquitous-language-invariants-scaffold.md` or an ADR. |
