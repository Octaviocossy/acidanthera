...
>
> **Last updated**: 2026-07-08 (Codex backend adapter, #16)
...
| AgentProcessState | `AgentProcessState` (`src-tauri/src/agent.rs`) | — | Rust-side `tauri::State`: the single running agent child process (if any) plus its stdin handle. `agent_spawn` replaces (kills) any process already running before starting the new one — mirrors `VaultState`'s watcher-replacement pattern. |
