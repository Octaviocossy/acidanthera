...
`settings.toml` and `keymaps.toml` are no longer surfaced as sidebar rows. ADR 0102 put them in two
...
ADR 0102's core decision — config lives outside the vault but is surfaced inside vault-scoped UI —
