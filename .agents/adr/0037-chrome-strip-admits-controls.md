...
ADR 0035 dissolved the titlebar into per-region *chrome strips* and ruled that the strip
...
the sidebar, which is why ADR 0035's substance survives this. The narrower rule is what keeps
the strip from drifting back into a titlebar one convenience at a time.

## Considered Options

- **Put the pair in the brand row instead**, leaving ADR 0035 untouched. Rejected: the brand
...
- The pair is expanded-sidebar only. At 40px the rail has no room, and unlike the gear
  (ADR 0035 decision 30) a navigation control with no visible destination is not worth a
